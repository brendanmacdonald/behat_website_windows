 ...
 */
class HelperContext implements SnippetAcceptingContext {
...
#    $this->generateRandomStrings();
  ...
  public function captureScreenAfterFailedStep(AfterStepScope $scope) {
    ...
    if ($driver instanceof \Behat\Mink\Driver\BrowserKitDriver) {
      //  Generate HTML dump.
      $html_data = $this->minkContext->getSession()->getDriver()->getContent();
  ...
  public function iWaitForSeconds($number) {
    $this->minkContext->getSession()->wait($number * self::WAIT);
  }
  ...
  public function iWaitForPageLoadToComplete() {
    ...
    while ((!$bool_page_load_complete) && ($counter < self::TIMEOUT)) {
      $page_state = $this->minkContext->getSession()
  ...
  public function waitForJquery() {
    $this->minkContext->getSession()->wait(self::TIMEOUT, '(0 === jQuery.active)');
  }
  ...
  public function iSelectTheModal($modal) {
    $this->minkContext->getSession()->switchToIFrame($modal);
  }
  ...
  public function iDeselectTheModal() {
    $this->minkContext->getSession()->switchToIFrame();
  }
...
JS;
    $result = $this->minkContext->getSession()->evaluateScript($javascript);
    return $result;
  }
...
JS;
    $this->minkContext->getSession()->executeScript($javascript);
  }
  ...
  public function iFillInFrameWith($targetFrameID, $text, $modalID = NULL) {
    ...
    // Select a frame.
    $this->minkContext->getSession()->switchToIFrame($targetFrameID);
  ...
  public function iClickElementWithClass($class) {
    $element = $this->minkContext->getSession()
      ->getPage()
  ...
  public function iFillInFieldByIDWith($field, $value) {
    ...
    $element = $this->minkContext->getSession()->getPage()->findById($field);
    if ($element) {
  ...
  public function iClickElementWithXpath($xpath) {
    $element = $this->minkContext->getSession()->getPage()->find('xpath', $xpath);
    if ($element) {
      $element->click();
    }
    else {
...
JS;
    try {
      $this->minkContext->getSession()->executeScript($function);
    } catch (CWContextException $e) {
  ...
  public function iSelectARandomEntryFromDropdown($dropdown) {
    $dropdownElement = $this->minkContext->getSession()->getPage()->findField($dropdown);
  ...
  public function iCheckTheHttpResponseCodeIsFor($expected_response, $url) {
    $path = $this->minkContext->getMinkParameter('base_url') . $url;
  ...
  public function iShouldGetTheFollowingHTTPStatusResponses(TableNode $table) {
    foreach ($table->getRows() as $row) {
      $this->minkContext->getSession()->visit($row[0]);
      $this->minkContext->assertSession()->statusCodeEquals($row[1]);
    }
  }
  ...
  public function iCanSeeTheLinkInTheRegion($link, $region) {
    $this->minkContext->assertElementOnPage($link);
  ...
  public function iCannotSeeTheLinkInTheRegion($link, $region) {
    $this->minkContext->assertElementNotOnPage($link);
  ...
  public function getHtml() {
    if (empty($this->html)) {
      $this->html = $this->minkContext->getSession()->getPage()->getHTML();
    }
    return $this->html;
  }
  ...
  public function iVerifyTheAssetsviaJS($assetType) {
    ...
    foreach ($assets as $asset) {
      ...
      elseif (preg_match('/^\//', $assetToCheck, $match)) {
        $url = $this->minkContext->getMinkParameter('base_url') . $assetToCheck;
      }
      else {
  ...
  public function getObjects($page) {
    // Go to page.
    $this->minkContext->visitPath($page);
